...
class Pixel {
  ...
  appear() {
    ...
    // Une fois le délai passé (ou zéro), afficher le pixel rapidement
    if (this.size < this.maxSize) {
      this.size = Math.min(this.size + this.sizeStep * 2, this.maxSize);
    }
    if (this.size >= this.maxSize) {
      this.isShimmer = true;
    }
...
export default function PixelCard({ variant = 'default', gap, speed, colors, noFocus, className = '', children }) {
  ...
  const initPixels = () => {
    ...
    // Si les dimensions sont invalides, utiliser les dimensions du container ou attendre
    if (width <= 0 || height <= 0) {
